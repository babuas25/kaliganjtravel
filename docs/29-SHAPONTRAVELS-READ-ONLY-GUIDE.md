...
The Redis quote is now written as binary deflate instead of Base64 text. This saves space without changing the Search result, RePrice checks, 20-minute TTL, or per-quote size cap. Existing Base64 quotes remain readable until they expire. On 2026-09-26 a read-only sample of three existing quotes measured 342,109 stored-value bytes; the same compressed payloads would require 256,582 binary bytes (about 25% less). The configured Redis reported about 2.64 MB in use, including Redis overhead. Actual 30 MB plan headroom and the number of simultaneous searches depend on other Redis keys and provider accounting; inspect `INFO memory` and `MEMORY USAGE` during real traffic rather than dividing the plan size by one quote.
